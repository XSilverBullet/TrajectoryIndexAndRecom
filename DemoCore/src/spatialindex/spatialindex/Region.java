...
public class Region implements IShape
{
	...
	public Region(final double[] pLow, final double[] pHigh)
	{
		...
		//(ԭ���飬ԭ�����е���ʼλ�ã�Ŀ�����飬Ŀ�������е���ʼλ�ã�Ҫ���Ƶ�����Ԫ�ص�����)
	...
	public boolean touches(final Point p)
	{
		...
		for (int cIndex = 0; cIndex < m_pLow.length; cIndex++)   //ά�ȵĵ�����x��y��z������
		{
	...
	public void combine(final Region r)
	{
		for (int cIndex = 0; cIndex < m_pLow.length; cIndex++)
		{
			this.m_pLow[cIndex] = Math.min(m_pLow[cIndex], r.m_pLow[cIndex]);
			this.m_pHigh[cIndex] = Math.max(m_pHigh[cIndex], r.m_pHigh[cIndex]);
		}
	}
	
	public void combine(final Point p)
	{
		for (int cIndex = 0; cIndex < m_pLow.length; cIndex++)
		{
			this.m_pLow[cIndex] = Math.min(m_pLow[cIndex], p.m_pCoords[cIndex]);
			this.m_pHigh[cIndex] = Math.max(m_pHigh[cIndex], p.m_pCoords[cIndex]);
		}
	}
